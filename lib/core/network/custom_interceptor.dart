...
class CustomInterceptor extends Interceptor {
  ...
  void _printPrettyMap(
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        logPrint('$color║${_indent(tabs0)} $key: {$reset');
        _printPrettyMap(value, tabs: tabs0, color: color);
      } else if (value is List) {
        logPrint('$color║${_indent(tabs0)} $key: [$reset');
        _printList(value, tabs: tabs0, color: color);
        logPrint('$color║${_indent(tabs0)} ]${isLast ? '' : ','}$reset');
      } else {
        logPrint('$color║${_indent(tabs0)} $key: $value${!isLast ? ',' : ''}$reset');
      }
    });
